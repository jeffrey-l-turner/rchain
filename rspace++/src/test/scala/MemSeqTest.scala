...
class MemSeqTest extends AnyFunSuite {
  ...
  test("MemSeqProduceMatch") {
    ...
    val cres        = lib.space_put_once_non_durable_sequential(spacePtr, receive_buf, receive_buf.length);

    // Produce
    val send     = Send("friends", Some(setup.alice), cityMatchCase(setup.alice));
    val send_buf = send.toByteArray;
    val pres     = lib.space_get_once_non_durable_sequential(spacePtr, send_buf, send_buf.length);
  ...
  test("MemSeqMultipleChannelsConsumeMatch") {
    ...
    val pres2     = lib.space_get_once_non_durable_sequential(spacePtr, send2_buf, send2_buf.length);
  ...
  test("MemSeqConsumePersist") {
    ...
    val send1_buf = send1.toByteArray;
    val pres      = lib.space_get_once_non_durable_sequential(spacePtr, send1_buf, send1_buf.length);
  ...
  test("MemSeqConsumePersistExistingMatches") {
    ...
    val pres3     = lib.space_get_once_non_durable_sequential(spacePtr, send3_buf, send3_buf.length);
  ...
  test("MemSeqProducePersist") {
    ...
    val send     = Send("friends", Some(setup.alice), cityMatchCase(setup.alice));
    val send_buf = send.toByteArray;
    val pres     = lib.space_get_always_non_durable_sequential(spacePtr, send_buf, send_buf.length);
  ...
  test("MemSeqProducePersistExistingMatches") {
    ...
    val send1     = Send("friends", Some(setup.alice), cityMatchCase(setup.alice));
    val send1_buf = send1.toByteArray;
    val pres1     = lib.space_get_always_non_durable_sequential(spacePtr, send1_buf, send1_buf.length);
    ...
    val send2_buf = send2.toByteArray;
    val pres2     = lib.space_get_always_non_durable_sequential(spacePtr, send2_buf, send2_buf.length);
