...
class MemConcTest extends AnyFunSuite {
  ...
  test("MemConcProduceMatch") {
    ...
    val cres        = lib.space_put_once_non_durable_concurrent(spacePtr, receive_buf, receive_buf.length);

    // Produce
    val send     = Send("friends", Some(setup.alice), cityMatchCase(setup.alice));
    val send_buf = send.toByteArray;
    val pres     = lib.space_get_once_non_durable_concurrent(spacePtr, send_buf, send_buf.length);
  ...
  test("MemConcMultipleChannelsConsumeMatch") {
    ...
    val pres1     = lib.space_get_once_non_durable_concurrent(spacePtr, send1_buf, send1_buf.length);

    // Produce
    val send2     = Send("friends", Some(setup.erin), stateMatchCase(setup.erin));
    val send2_buf = send2.toByteArray;
    val pres2     = lib.space_get_once_non_durable_concurrent(spacePtr, send2_buf, send2_buf.length);
  ...
  test("MemConcConsumePersist") {
    ...
    val receive_buf = receive.toByteArray;
    val cres =
      lib.space_put_always_non_durable_concurrent(spacePtr, receive_buf, receive_buf.length);
    ...
    val send1_buf = send1.toByteArray;
    val pres      = lib.space_get_once_non_durable_concurrent(spacePtr, send1_buf, send1_buf.length);
  ...
  test("MemConcConsumePersistExistingMatches") {
    ...
    val cres1 =
      lib.space_put_always_non_durable_concurrent(spacePtr, receive1_buf, receive1_buf.length);
    ...
    val cres2 =
      lib.space_put_always_non_durable_concurrent(spacePtr, receive2_buf, receive2_buf.length);
    ...
    val cres3 =
      lib.space_put_always_non_durable_concurrent(spacePtr, receive3_buf, receive3_buf.length);
    ...
    val pres3     = lib.space_get_once_non_durable_concurrent(spacePtr, send3_buf, send3_buf.length);
  ...
  test("MemConcProducePersist") {
    ...
    val send     = Send("friends", Some(setup.alice), cityMatchCase(setup.alice));
    val send_buf = send.toByteArray;
    val pres     = lib.space_get_always_non_durable_concurrent(spacePtr, send_buf, send_buf.length);
  ...
  test("MemConcProducePersistExistingMatches") {
    ...
    val receive1_buf = receive1.toByteArray;
    val cres1 =
      lib.space_put_once_non_durable_concurrent(spacePtr, receive1_buf, receive1_buf.length);
    ...
    val send1     = Send("friends", Some(setup.alice), cityMatchCase(setup.alice));
    val send1_buf = send1.toByteArray;
    val pres1     = lib.space_get_always_non_durable_concurrent(spacePtr, send1_buf, send1_buf.length);
    ...
    val send2_buf = send2.toByteArray;
    val pres2     = lib.space_get_always_non_durable_concurrent(spacePtr, send2_buf, send2_buf.length);
    ...
    val receive2_buf = receive2.toByteArray;
    val cres2 =
      lib.space_put_once_non_durable_concurrent(spacePtr, receive2_buf, receive2_buf.length);
