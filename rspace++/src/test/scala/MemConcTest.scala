...
class MemConcTest extends AnyFunSuite {
  ...
  test("MemConcProduceMatch") {
    ...
    lib.space_put_once_non_durable_concurrent(spacePtr, receive_buf, receive_buf.length);

    // Produce
    val send     = Send("friends", Some(setup.alice), cityMatchCase(setup.alice));
    val send_buf = send.toByteArray;
    lib.space_get_once_non_durable_concurrent(spacePtr, send_buf, send_buf.length);
  ...
  test("MemConcMultipleChannelsConsumeMatch") {
    ...
    lib.space_get_once_non_durable_concurrent(spacePtr, send1_buf, send1_buf.length);

    // Produce
    val send2     = Send("friends", Some(setup.erin), stateMatchCase(setup.erin));
    val send2_buf = send2.toByteArray;
    lib.space_get_once_non_durable_concurrent(spacePtr, send2_buf, send2_buf.length);
  ...
  test("MemConcConsumePersistExistingMatches") {
    ...
    lib.space_get_once_non_durable_concurrent(spacePtr, send3_buf, send3_buf.length);
  ...
  test("MemConcProducePersist") {
    ...
    val send     = Send("friends", Some(setup.alice), cityMatchCase(setup.alice));
    val send_buf = send.toByteArray;
    lib.space_get_always_non_durable_concurrent(spacePtr, send_buf, send_buf.length);
  ...
  test("MemConcProducePersistExistingMatches") {
    ...
    val send1     = Send("friends", Some(setup.alice), cityMatchCase(setup.alice));
    val send1_buf = send1.toByteArray;
    lib.space_get_always_non_durable_concurrent(spacePtr, send1_buf, send1_buf.length);
    ...
    val send2_buf = send2.toByteArray;
    lib.space_get_always_non_durable_concurrent(spacePtr, send2_buf, send2_buf.length);
